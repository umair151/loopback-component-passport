...
PassportConfigurator.prototype.buildUserLdapProfile = function (user, options) {
  ...
  for (var profileAttrName in options.profileAttributesFromLDAP) {
    ...
    if (ldapAttrName === "objectGUID" || ldapAttrName === "objectSid") {
      if (user[ldapAttrName].length == 14) {
        profile[profileAttrName] = bytesToUuid(Array.prototype.slice.call(Buffer.from(user[ldapAttrName])));
      }
    }
  ...
  if (!profile.id) {
    profile.id = user["uid"] || bytesToUuid(Array.prototype.slice.call(Buffer.from(user["objectGUID"])));
  }
  if (!profile.emails) {
...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  var successRedirect = function (req, accessToken) {
    ...
    return appendAccessToken(options.successRedirect, accessToken) || (link ? "/link/account" : "/auth/account");
  };
            ...
            if (user) {
              ...
              if (options.setAccessToken) {
                ...
                return user.hasPassword(password, function (err, ok) {
                  ...
                  // security purposes.
                  if (ok && user.emailVerified) return done(null, userProfile);
    ...
    case "saml":
      strategy = new AuthStrategy(
        _.defaults(
          {
            ...
            privateCert: options.privateCertPath
              ? fs.readFileSync(options.privateCertPath, "utf-8")
              : options.privateCert,
            decryptionPvk: options.decryptionPvkPath
              ? fs.readFileSync(options.decryptionPvkPath, "utf-8")
        ...
        function (req, profile, done) {
          ...
          profile.id =
            profile.id || profile.nameID || profile["http://schemas.microsoft.com/identity/claims/objectidentifier"];
          if (link) {
            if (req.user) {
